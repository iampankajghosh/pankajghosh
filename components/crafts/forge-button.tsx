...
const buttonVariants = cva(
  "group/button relative inline-flex shrink-0 items-center justify-center overflow-hidden whitespace-nowrap rounded-lg border border-transparent bg-neutral-800 bg-clip-padding font-geist text-sm font-medium text-white outline-none select-none  will-change-transform transition-[transform,opacity,background-color,color,border-color,box-shadow,filter] duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:transition-none motion-reduce:transform-none focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.97] active:duration-75 active:ease-out cursor-pointer ring-1 ring-neutral-300 before:absolute before:inset-0.5 before:-z-10 before:rounded-md before:bg-transparent before:content-[''] before:inset-shadow-2xs before:inset-shadow-neutral-400 after:absolute after:inset-0 after:-z-20 after:m-px after:rounded-lg after:bg-neutral-700 after:content-[''] after:inset-shadow-sm after:inset-shadow-neutral-900 after:filter-[blur(0.4px)] hover:brightness-[1.03] dark:ring-neutral-800 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      size: {
        default:
